...
contract('DappUniversityToken', ([owner, minter, pauser, holder1, holder2]) => {
  let token
  const _name = "Dapp University"
  const _symbol = "DAPPU"

  beforeEach(async () => {
    token = await DappUniversityToken.new()
  })
  ...
  describe('roles', async () => {
    it('only owner can add/remove roles', async () => {
      await token.addMinter(minter, { from: owner }).should.be.fulfilled
      await token.addMinter(pauser, { from: minter }).should.be.rejected
    })
    ...
    it('only pauser role can pause the token', async () => {
      await token.pause({ from: minter }).should.be.rejected
