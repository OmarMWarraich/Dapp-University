  ...
  try {
    ...
    for(let i=0; i<files.length; i++){
      if(network === 'Optimistic Kovan' || network === 'Optimistic Mainnet') {
        await nft.mint(`https://ipfs.io/ipfs/${upload.cid.toString()}/${files[i]}`, web3.utils.toWei('0.01', 'Ether'), {gas:15000000})
      } else {
        await nft.mint(`https://ipfs.io/ipfs/${upload.cid.toString()}/${files[i]}`, web3.utils.toWei('0.01', 'Ether'), {gasPrice: 0})
      }
