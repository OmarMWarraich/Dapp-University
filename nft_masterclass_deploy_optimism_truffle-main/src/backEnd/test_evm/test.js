...
console.log(`Testing evm contracts`)
const ether = n => web3.utils.toWei(n.toString(), 'ether').toString()

console.log("Testing on EVM")
...
contract('NFT', ([acc1, acc2]) => {
  ...
  describe('deploy, mint, buy and test...', () => {
    ...
    beforeEach(async () => {
      await nft.mint('token_uri_1', ether(0.01))
      await nft.mint('token_uri_2', ether(0.01)) 
    ...
    it("+ test if rejects buying for invalid id, msg.value and status", async () => {
      expectRevert(nft.buy('3', {from: acc2, value: ether(0.01)}), "Error, wrong Token id")
      expectRevert(nft.buy('2', {from: acc2, value: ether(0.01)}), "Error, Token costs more")
      expectRevert(nft.buy('1', {from: acc2, value: ether(0.02)}), "Error, Token is sold")
    })
  })
})